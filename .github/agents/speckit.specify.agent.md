...
Before creating the branch or writing the specification:

1. Check whether `.specify/context/selected-pbi.json` exists in the repository root.
2. If it exists, read and parse it.
3. If parsing fails, continue silently using only user input.
4. If parsing succeeds, treat `pbi` as authoritative context and merge it into the spec source input.
5. Use this context to enrich the generated specification with traceability details.

When PBI context is present, include these details in the generated spec:
- PBI ID (`taskId` and numeric `id`)
- Title
- Description
- Acceptance Criteria
- State
...
If user input and PBI context conflict, prefer explicit user input for scope and constraints, but keep PBI fields as traceability metadata.
